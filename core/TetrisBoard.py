...
class TetrisBoard:
    ...
    def add_row(
        ...
        if len(row) != self.width:
            raise ValueError(f"Row must have the same width of the board ({self.width})")
    ...
    def get_first_empty_row_from_bottom_to_top(
        self,
        column_idx: int,
    ) -> int | None:
        ...
        for row in range(block.get_height()):
            for col in range(block.get_width()):
                ...
                if 0 <= target_row < len(self.board) and 0 <= target_col < len(self.board[0]):
                    self.board[target_row][target_col] = block.get_value_at(
                        row,
                        col,
                    )
    ...
    def drop_block_at_column(
        ...
        if col_idx > self.width:
            print("EXCEPTION")
            raise ValueError(f"Column index must be less than the board's width  ({self.width})")
    ...
    def check_for_filled_rows(
        self,
    ):
        return

    def get_sub_matrix_from_board(
        board: Deque,
        x: int,
        y: int,
    ):
